  ...
  render() {
    ...
    if (transition === PUSH) {
      previousChildren && routes.push(
        <Animated.View key={previousChildren.key} style={this.getRouteStyle(0)}>
          {(!this.isFixed("footer") && PreviousFooter) && <PreviousFooter />}
          {previousChildren}
          {(!this.isFixed("header") && PreviousHeader) && <PreviousHeader />}
        </Animated.View>
      );
      children && routes.push(
        <Animated.View key={children.key} style={this.getRouteStyle(1)}>
          {(!this.isFixed("footer") && Footer) && <Footer />}
          {children}
          {(!this.isFixed("header") && Header) && <Header />}
        </Animated.View>
      );
    ...
    return (
      <View key="stack-view" style={[styles.stackView, stackViewStyle]} {...this.panResponder.panHandlers}>
        { this.isFixed("footer") && <Footer />}
        <View style={styles.transitionContainer}>
          {routes.map(route => route)}
        </View>
        { this.isFixed("header") && <Header />}
      </View>
    );
  }
}
